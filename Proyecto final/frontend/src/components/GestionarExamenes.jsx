  ...
  return (
    <div className="gestion-examenes">
      <div className="barra-superior">
        <button className="btn-volver" onClick={volverAEvaluaciones}>
      ...
      {examenes.length === 0 ? (
        ...
        examenes.map((examen) => (
          <div className="examen-item" key={examen._id}>
            <h3>{examen.title}</h3>
            {examen.creatorName && (
              <p><strong>Docente:</strong> {examen.creatorName}</p>
            )}
            {examen.date && (
              <p><strong>Fecha:</strong> {new Date(examen.date).toLocaleDateString()}</p>
            )}
            <button className="edit" onClick={() => navigate(`/editar-examen?id=${examen._id}`)}>
              Editar
            </button>
            <button className="delete" onClick={() => eliminarExamen(examen._id)}>
              Eliminar
            </button>
          </div>
        ))
